...
pub(crate) fn check_lens(name: &str, a: usize, b: usize) {
    if a != b {
...
pub(crate) fn l2sq_f32(a: &[f32], b: &[f32]) -> f32 {
    #[cfg(feature = "jit")]
    if let Some(k) = crate::compile::jit::simd::kernels() {
...
pub(crate) fn add_f32_into(a: &[f32], b: &[f32], out: &mut Vec<f32>) {
    out.clear();
...
pub(crate) fn scale_f32_into(a: &[f32], k_val: f32, out: &mut Vec<f32>) {
    out.clear();
    out.resize(a.len(), 0.0);
    #[cfg(feature = "jit")]
    if let Some(k) = crate::compile::jit::simd::kernels() {
...
/// magnitude is zero.
pub(crate) fn norm_f32_into(a: &[f32], out: &mut Vec<f32>) {
...
pub(crate) fn lid_mle_of(distances: &[f32], k: f64) -> f64 {
...
pub(crate) fn hash_vec_into(seed: u64, dim: u64, out: &mut Vec<f32>) {
...
pub(crate) fn xxhash3_vec_into(seed: u64, dim: u64, out: &mut Vec<f32>) {
    let dim = dim as usize;
    out.clear();
    out.reserve(dim);
    for i in 0..dim {
