...
mod tests {
    use super::*;
    use crate::iteration::source::{Cursors, DataSourceFactory, RangeSourceFactory};
...
out := u64_add(m, k)
";
        crate::dsl::compile::compile_polydat_tier1_simd_ordinal(source, "base__ordinal", "out")
            .unwrap()
    }
...
out := u64_add(m, k)
";
        let mut executor = crate::dsl::compile::compile_polydat_to_assembler(source)
            .unwrap()
...
";
        let error = crate::dsl::compile::compile_polydat_to_assembler(source)
            .unwrap()
            .try_compile_tier1_simd_ordinal("base__ordinal", "out")
            .err()
