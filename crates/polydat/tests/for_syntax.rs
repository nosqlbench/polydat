...
fn producer_binding_parses_to_a_comprehension() {
    ...
    let Statement::Binding(b) = &f.statements[0] else { panic!("expected binding") };
    assert_eq!(b.targets, vec!["sweep"]);
    let Expr::For(source) = &b.value else { panic!("expected for expression, got {:?}", b.value) };
...
fn traversal_over_inline_comprehension_with_body() {
    ...
    let f = parse(src);
    let Statement::For(fs) = &f.statements[0] else { panic!("expected for statement") };
    assert_eq!(fs.source.text, "phase in load,verify, p in partitions(\"*/4\", 1000000)");
...
fn traversal_over_bound_producer() {
    ...
    let Statement::For(fs) = &f.statements[1] else { panic!("expected for statement") };
...
fn nested_traversals_and_every_statement_kind_in_a_body() {
    ...
    let f = parse(src);
    let Statement::For(outer) = &f.statements[0] else { panic!() };
    assert_eq!(outer.body.len(), 5);
    let Statement::For(inner) = &outer.body[4] else { panic!("expected nested for") };
...
fn where_and_order_survive_in_the_captured_text() {
    let f = parse("for k in 1..4, limit in 10,20,30 where {k} >= 2 && {limit} != 20 order extrema/1 {\n    x := hash(k)\n}\n");
    let Statement::For(fs) = &f.statements[0] else { panic!() };
...
fn comments_after_the_comprehension_are_not_captured() {
    ...
    let Statement::Binding(b) = &f.statements[0] else { panic!() };
    let Expr::For(source) = &b.value else { panic!() };
