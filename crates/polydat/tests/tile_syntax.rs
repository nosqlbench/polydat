    ...
    pieces
        ...
        .map(|p| match p {
            ...
            TilePiece::Hole(h) => format!(
                "H({}{}{}{})",
                h.text,
                h.decl_type.as_ref().map(|t| format!(" :{t}")).unwrap_or_default(),
                h.format.as_ref().map(|f| format!(" |{f}")).unwrap_or_default(),
                if h.raw { " !" } else { "" }
            ),
            TilePiece::Projection { source, sep, body, .. } => format!(
                "P({}{} [{}])",
                source.text,
                sep.as_ref().map(|s| format!(" sep {s:?}")).unwrap_or_default(),
                shape(body)
            ),
            TilePiece::Branch { then, otherwise, .. } => format!(
                "B([{}]{})",
                shape(then),
                otherwise.as_ref().map(|o| format!(" else [{}]", shape(o))).unwrap_or_default()
            ),
        })
        .collect::<Vec<_>>()
        .join(" ")
...
fn string_literal_body_and_csv_encoding() {
    ...
    assert_eq!(shape(&t.pieces), "H(tenant_id) S(\",\") H(device_id) S(\",\") H(ts)");
}

#[test]
fn hole_modifiers_type_format_and_raw() {
    let t = tile("tile t := \"${ts: u64} ${temp_c + s | .2} ${doc!} ${a | 05} ${b: str | >8 !}\"\n");
    assert_eq!(
        shape(&t.pieces),
        "H(ts: u64 :u64) S(\" \") H(temp_c + s | .2 |.2) S(\" \") H(doc! !) S(\" \") H(a | 05 |05) S(\" \") H(b: str | >8 ! :str |>8 !)"
    );
    let TilePiece::Hole(h) = &t.pieces[2] else { panic!() };
    ...
    let TilePiece::Hole(h) = &t.pieces[0] else { panic!() };
...
fn projections_with_default_and_explicit_separators_and_nesting() {
    let t = tile("tile doc : json {\"samples\": [ @for s in 0..4 { { \"n\": ${s}, \"t\": ${temp + s} } } ], \"tags\": [ @for t in a,b sep \"; \" { \"${t}\" } ]}\n");
    assert_eq!(
        shape(&t.pieces),
        "S(\"{\\\"samples\\\": [ \") P(s in 0..4 [S(\"{ \\\"n\\\": \") H(s) S(\", \\\"t\\\": \") H(temp + s) S(\" }\")]) S(\" ], \\\"tags\\\": [ \") P(t in a,b sep \"; \" [S(\"\\\"\") H(t) S(\"\\\"\")]) S(\" ]}\")"
    );
    let TilePiece::Projection { source, .. } = &t.pieces[1] else { panic!() };
    ...
    assert!(matches!(&t.pieces[0], TilePiece::Projection { body, .. } if matches!(&body[0], TilePiece::Projection { .. })));
}

#[test]
fn projection_over_a_bound_producer_and_a_where_predicate() {
    let t = tile("tile n := \"@for sweep { ${k} } @for k in 1..9 where {k} > 3 { ${k} }\"\n");
    let TilePiece::Projection { source, .. } = &t.pieces[0] else { panic!() };
    assert!(matches!(&source.kind, polydat::dsl::ast::ForSourceKind::Producer(p) if p == "sweep"));
    let TilePiece::Projection { source, .. } = &t.pieces[2] else { panic!() };
    assert!(source.text.contains("where {k} > 3"));
}

#[test]
fn branches_with_and_without_else() {
    let t = tile("tile b := \"@if verbose { , \\\"audit\\\": \\\"${operator}\\\" } tail @if x > 1 { a } @else { b }\"\n");
...
fn custom_delimiters_and_sigil() {
    let t = tile("tile t : text (delims \"<%\" \"%>\", sigil \"#\") := \"a <%x%> b #for k in 1..3 { <%k%> } ${literal} #if c { y }\"\n");
...
fn braces_inside_hole_expressions_and_strings_do_not_close_holes() {
    ...
    assert_eq!(shape(&t.pieces), "H(if x > 1 { 1 } else { 2 }) S(\" \") H(str_concat(\"}\", y))");
}
...
fn errors_name_the_tile_and_the_problem() {
    ...
    assert!(e.contains("tile 't'") && e.contains("unterminated hole"), "{e}");
...
fn compiler_lowers_a_tile_to_a_wire() {
    let mut k = polydat::dsl::compile_polydat("input cycle: u64\ntile t := \"n=${cycle}\"\n").unwrap();
    k.set_inputs(&[4]);
