...
Since engine parity step 9, every engine a host can choose accepts every
program the interpreter accepts, and computes what the interpreter computes
([engine parity](engine_parity.md); the node-by-engine matrix in the
[node reference](../reference/nodes.md) is generated by the parity suite).
The remaining distinctions are placements inside an engine, not refusals:
