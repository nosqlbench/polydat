...
element — by `iteration::comprehension::eval::evaluate_spec(spec_text,
&dyn Lookup)` against a `Lookup` view of the enclosing scope (a
`PolydatKernel` or a `Layered` prefix over it). The only structural
operation resolution performs is **peeling exactly one level**:
...
/ `polydat_core::iteration::comprehension::source_values::strip_string_tokens`,
double-quoted only): split on runs of
...
The figure `ir::bounds::check_bounds` reports for an
`ORDER_MATERIALIZE` working set is the truncation cap, a lower
bound on the working set (the actual is the input cardinality
when push-down does not apply).
...
struct EvaluatedSource {
  ...
  /// How many values — `values.len()` for any
  /// successfully-evaluated discrete source; zero for an
  /// unsampled continuous source, whose `index_fn` is
  /// `Continuous` and which requires an enclosing sampler
  /// under V8.
  cardinality: u64,
...
struct EvalContext<'a> {
  var_name: &'a str,                 // the clause's element name, for messages
  scope: &'a dyn Lookup,             // where the source's names resolve
  prefix: &'a [(String, Value)],     // prior-axis bindings, in axis order
}
...
```text
analyze_reducibility(ast: &Comprehension) -> ReducibilityFinding

struct ReducibilityFinding {
  reduction: Option<Reduction>,     // None is the empty finding
  rule: Option<RuleId>,             // which rule fired
  improvement: ComplexityDelta,     // asymptotic improvement of the witness
}
```

- `ast`: the AST node under consideration (and, recursively, its
  children).
- The metadata bundle for `ast`, propagated per §10.7, is read
  through `ast.metadata()`. The analyzer reads it exhaustively
  but **never reads `ast` beyond what metadata surfaces** —
  that is, no peeking into Polydat expression internals,
  source-expression internals, or any field of `ast` that
  §10.7's propagation rules did not publish into metadata. For
  predicate-shape facts the analyzer consults the predicate
...
```rust
// `sweep` is the StreamerValue the wire carries; `compiled()` is
...
let scoped = compiled.scope_once(&parent, &replay_coords);
let result = run(&scoped.scoped);
```
...
- `polydat_grammar::comprehension::ast::Comprehension` is the
  canonical six-variant operator tree. The grammar crate's
  comprehension modules (`ast`, `parse`, `spec`, `metadata`,
  `cardinality`, `strategy`, `source`) are re-exported under
...
- The text front end (`polydat_grammar::comprehension::parse::parse_comprehension_text`,
  ...
  (both `polydat_grammar::comprehension::spec`) are the same
  normalization for text and serde input handed in directly.
