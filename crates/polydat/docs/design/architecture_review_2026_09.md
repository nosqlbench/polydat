...
| engine_parity.md | none | **Delete** after its rules move to engines.md §3/§6/§7, for_traversal.md, and the None rule. Its method (matrix from coverage cases written into the node reference) becomes one paragraph in engines.md §7. |
...
| scope_model.md | What a scope is and how one is constructed | **Rewrite** §1, §2, §4, §8 to kernels on any engine, `create_kernel`/`create_nested_kernel`, the binder's steps in present tense. Add the traversal cascade rule once decided. |
