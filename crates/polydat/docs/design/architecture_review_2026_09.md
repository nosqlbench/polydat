...
- **Group A, what is left of the second half** — reconciling `run_fresh` and `run_order`,
  which differ because the step type does, and the two `build_hybrid`s. `run_guarded`
  turned out to be byte-identical and is shared as of 2026-09-21 with the rest of the
  safe half. Gated on the ladder, so it waits for a quiet machine.
