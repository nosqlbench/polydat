...
group F, and all of group E but F-K5(c) have landed, and the runtime, node library, and language
...
- **Group E, one part** — F-K5(c), materialization over `dyn Kernel`. Blocked on §4's
  ...
  is closed.
- **Group G** — one macro kit. Cross-area with the JIT.

**Group F closed 2026-09-20/21.** Of section 5's decisions, all eight are taken; 3 (the
engine projection bodies use) fell out of group F: a projection body runs on the engine
of the kernel that renders the tile, as a `for` body runs on the engine of the kernel
that opened it.
