...
- **Group A, second half** — reconciling `run_fresh`, `run_guarded` and `run_order`, the
  ...
  for a quiet machine.
- **Group F** — F-L2 and F-L4, which are one change: the compiler hands `tile_render` its
  projection body as source text inside a JSON string, so the body cannot cross as a
  compiled program until that handoff carries something other than text. F-L5 and half of
  F-C13 are done.
- **Group E** — one binder for child kernels. Depends on F for the tile side.
- **Group G** — one macro kit. Cross-area with the JIT.

Of section 5's decisions, 1, 2, 4, 5, 6, 7 and 8 are taken; 3 (the engine projection
bodies use) is open and falls out of group F.
