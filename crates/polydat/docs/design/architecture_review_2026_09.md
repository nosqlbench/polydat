...
group F, all of group E but F-K5(c), and group G's dead half have landed, and the
...
- **Group A, one measurement and one larger change** — `run_fresh` and `run_order` are
  written and tested on the `group-a-hot-loops` branch and are not measured, so they are
  not on main; the machine went from 94% load to 58% during the attempt and read 549 ns
  for a `p1_interpreter` that is 373 ns quiet. What is left after them is collapsing the
  two `build_hybrid`s into one, which needs `JitOp` to exist in a build without the JIT
  feature so the no-JIT path can be that builder with every node classified `Fallback`.
  Everything else in the group landed 2026-09-21.
- **Group E, one part** — F-K5(c), materialization over `dyn Kernel`. Blocked on §4's
  ...
  pull path and so waits for the ladder alongside group A. Everything else in the group
  is closed.
