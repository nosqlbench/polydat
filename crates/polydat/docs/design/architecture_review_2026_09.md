...
- **Group A, second half** — reconciling `run_fresh`, `run_guarded` and `run_order`, the
  three loops that differ because the step type does. Gated on the ladder, so it waits
  for a quiet machine.
