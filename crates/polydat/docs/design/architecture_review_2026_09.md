...
Recorded 2026-09-11. Status as of 2026-09-21: the step-1 fixes (F-C1, F-N3, F-N11, F-E4,
F-E5, F-E13 stated, F-C12 documented, F-K2), F-K1, F-E3, F-E6, groups B, C, I, L,
group A, group F, all of group E but F-K5(c), and group G's dead half have landed, and the
runtime, node library, and language have since been split into their own crates; section 3's weeding map is closed, every
...
Group D closed with it: one write rule checked on every engine, and the interpreter's
borrowing readers renamed to `pull_ref`/`pull_ref_at` so they no longer shadow the trait.
The functional table's rows were re-checked against the code rather than trusted, and the
ones that had landed without being marked are now marked.

**Every functional finding is closed**, and groups B, C, D, H, I, J, K, L and M with them.
What remains:

- **Group A is closed**, with `run_fresh` and `run_order` left as they are and a
  measurement saying why: a shared loop costs the closure tier 5.1% and 6.9% on the two
  P2 rungs and the hybrid 3.1% on `p3_native`, against a bracketed baseline on a quiet
  machine, with `p1_interpreter` flat so the signal is the loops and not the binary. The
  one thing anyone might still do is collapse the two `build_hybrid`s into one, which
  needs `JitOp` to exist in a build without the JIT feature so the no-JIT path can be
  that builder with every node classified `Fallback`.
- **Group E, one part** — F-K5(c), materialization over `dyn Kernel`. Blocked on §4's
  item 12, output broadcast cells on the compiled engines, which lands in the compiled
  pull path and so wants the ladder. Everything else in the group is closed.
- **Group G, the reshaping** — one macro kit, the kit-selection ladder written four
  times, and the positional `jit_constants` contract. The dead half (F-N2's two unused
  type-to-slot mappings, F-N7's ninety-two names that are not nodes) landed 2026-09-21;
  what is left touches the closure, hybrid and JIT builders, so it wants the ladder
  with it.
...
**Group A closed 2026-09-21**, the last of the group work: the ladder was run again on a
quiet machine, rejected the one change that would have unified the two hot loops, and the
row carries the numbers. What is left across the review is F-K5(c), group G's reshaping,
and one builder rather than two. **Group F closed
...
renders the tile, as a `for` body runs on the engine of the kernel that opened it, which
holds on the interpreter path and needs `compiled_slot` to learn its engine to hold on
the compiled ones.
...
| ID | Area | Size | Defect |
|---|---|---|---|
| F-K1 | kernel | S | Constructing or compiling an interpreter kernel opened a root cycle (the build's constant fold seeded inputs through `set_inputs`), which reset the thread's arena. **Superseded**: the cycle and the arena were removed with SRD 115's third revision (runtime_model.md R4), so there is no cycle for a build to open and no arena for it to reset. |
...
| F-H3 | host | S | The assembler's strict checks (implicit adapters, config-wire violations) run only in `compile_strict`, which only the interpreter path reaches. `strict: true` on a compiled engine accepts what the interpreter rejects. **Closed 2026-09-20** with group B: `refuse_strict` → `strict_violation` is one function every compiled arm calls, so an implicit adapter is refused identically on all four engines. |
...
| F-H4 | host | S | The two diagnostic entry points build a different program from the one they diagnose (one rejects `for`, one skips pragmas and source text). **Closed 2026-09-20** with group B: `compile_polydat_checked` goes through `compile_ast_with_engine` with the source and the default options, so it neither skips the pragmas nor rejects a `for`. Writing the cross-check found a real divergence beyond the finding — the validation pass asked the registry whether a call resolved, and a module the program itself defines is not in the registry, so `d(a: u64) -> (o: u64) := {…}` followed by `x := d(cycle)` was reported as an unknown function while every compile entry point accepted it. A program's own modules take the call first now. The test runs the two against the same program across six shapes. |
...
| F-N8 | library | S | `dynamic_weighted_select` re-parses its spec every evaluation; `random_string` re-parses its charset per call; every `Const<Vec<C>>` node clones its list per cycle. **Closed 2026-09-20.** Two were already fixed and checked rather than assumed: `dynamic_weighted_select` memoizes the parsed spec and its alias table in the evaluating kernel state, and only the stateless path — a node evaluated with no state to memoize in — still parses per call, which is what that path is; `random_string` expands its charset once at construction through `#[poly_const]`. The third was live and needed the macro to grow a shape: `Const<&[C]>`, the borrowed const list, which the library now uses for `mixed_radix`, `is_one_of`, `one_of`, `reg_shuffle_bytes`, the alias sampler's weights and the three `fixed_values_*`, so the body sees the stored list rather than a clone of it. |
...
|---|---|---|---|
| A. One compiled core | F-E1, F-E2, F-E10, F-E8, F-E11 | L | The closure tier and the hybrid kernel are one engine written twice (same thirty fields, same twenty methods; the no-jit hybrid is a third copy). Eleven provenance-mode types carry one flag. One core, one step enum `{Native, Closure, Copy}`, provenance as a field with an optional cone guard; the closure engine is the same builder with every node classified fallback. The pure tier keeps a raw kernel for the differential and Tier-1 and drops the rest. Net deletion around 1,500 lines. Gated by the ladder. **Measured 2026-09-19**, which splits the group in two. Of the 23 methods the two cores share by name, **17 bodies are byte-identical** and two more differ only cosmetically (`ref_entry` by an import path, `plan` because the hybrid counts native segments the closure tier has none of). The genuine divergence is four methods: `run_fresh`, `run_guarded`, `run_order`, and `validate_refs`, and the first three are the hot loops, which differ because the step type does. So the safe half is sharing ~19 bookkeeping methods and the ~24 fields they read, which touches no evaluation path; the half that needs the ladder is reconciling three loops. `Kernel::engine` was made state rather than type on 2026-09-19, which is what lets one type serve both tiers. **Re-measured 2026-09-21, and most of the numbers moved.** The two cores carry 29 fields with the same names and exactly one differs in substance — `steps`, `Arc<[CompiledStep]>` against `Arc<Vec<HybridStep>>`; two more differ by an import path, and the hybrid has four of its own. The step type is therefore the whole of the divergence, which is what makes `run_order` and `run_fresh` differ and nothing else. Of the methods the cores define directly, `ref_entry` and `run_guarded` are byte-identical and now shared, leaving `failing_node`, `plan`, `run_order` and `run_fresh`. The larger duplication turned out not to be between the cores at all but among the **seven** kernel wrapper types (not eleven): the closure tier already shared eight accessors through `kernel_accessors!`, the hybrid hand-wrote the same eight three times each, and all seven wrote `pull_output` identically. And there is one `HybridCore`, not a third copy for a build without the JIT — what is duplicated there is `build_hybrid`, 276 lines against 102. **The safe half landed 2026-09-21**, 237 lines net, no evaluation path touched: the macro moved to `compile::mod` and all seven types take it, with `set_input`/`set_input_at`/`mark_all_dirty` left per tier because the two genuinely spell them differently. The ladder earned its gate — the shared `run_guarded` dropped the `#[inline]` both tiers had, `p3_native` paid seven percent for it twice against a same-window baseline, and restoring it put the rung back at 37.6 ns. That is the second time this attribute has gone missing into a shared macro, so it now carries a comment saying it is load-bearing. **The builders' shared block landed the same day**: each carried the same thirty-five lines for "make a closure step out of node i", differing in where the node's slots came from and in the wording of the refusal, and that is `closure_step_for` over a `GraphView` now. Build-time code, so not ladder-gated. **The two hot loops stay written twice, and this is the measurement that decided it.** They were shared on `group-a-hot-loops`, which is kept as the record: each tier writes `step_flags`, `step_count` and `run_one` for itself, `#[inline(always)]`, and the loop bodies move into the macro. It compiles, it passes both legs of the suite, and on a quiet machine with a bracketed baseline (main measured before and after) it costs **+5.1% on `p2_closures`, +6.9% on `p2_closures_dyn`, and +3.1% on `p3_native`**, with `p1_interpreter` flat at −0.8% — so the signal is the loops and not the binary's layout. Collapsing two hooks into one to save an index did not help (95.8 ns against 95.0, where main is 90.3), which locates the cost: not the number of indexes but reaching the step list through a call at all. The hand-written loops hoist `let steps = &self.steps` out of the loop and index a local slice; a shared loop must go back through `self` and deref the `Arc` for every step. **The group's premise is answered rather than acted on.** The two loops differ because the step type does, and the step type differs because a native segment stands for a run of nodes while a closure step is one node — so the closure tier keeps a step's flags on the step and the hybrid keeps them in arrays beside it, and each is right for its tier. Unifying them is not free and the ladder is what says so. **What is left** is one builder rather than two, which needs `JitOp` to exist without the `jit` feature so the no-JIT path can be the same builder with every node classified `Fallback` — the group's original proposal, and larger than anything here. |
| B. One entry point | F-H1, F-H2, F-C3, F-C2, F-C15, F-H9 | M | Twenty-four `compile_*` functions, six of them full pipelines with different settings; twenty-two assembler constructors with a fallback that returns an empty kernel or panics. One `Compiler::from_options`, one `compile_file` shared by the interpreter and compiled paths, four public conveniences, one `KernelError`; everything else a deprecated two-line wrapper. **Closed 2026-09-20; mostly done 2026-09-18**: the entry points are sixteen dividing by what they return, each named for it, every one on `KernelError`, the fourteen deprecated wrappers deleted rather than left to be found. The empty-kernel fallback and the `expect` on a public method are gone. The assembler's builders followed: the four with no caller and the one that was a pure alias of `compile_hybrid` are deleted, leaving eight, and the family is `#[doc(hidden)]` throughout, which is what §1 already said it was. Deleting them made `JitKernelPush` and `JitKernelPull` unconstructible, so the pure tier's two unreachable provenance variants went with them — two of the eleven mode-carrying types group A names, removed by the group that exposed them.  Its four folded-in findings closed with it: F-H3 and F-H4 (one strict check and one diagnostic path on the same build), F-C4, and F-L3. `compile_file_with` is the one pipeline the interpreter and the compiled engines share, and `Compiler` carries the options rather than each entry point rebuilding them. |
...
| E. One binder for child kernels | F-K5, F-K11, F-K12, F-L9, F-C6 | L | Subcontext materialization (interpreter only, cells and transit) and traversal activation (any engine, snapshot) are two mechanisms for one idea. Express binding over `dyn Kernel` plus a `Lookup` view; streams remember the engine that opened them; `Lookup` implemented for a layered constant view so opening a traversal allocates no interpreter state; the kernel-bound evaluation surfaces take `&dyn Lookup`. **F-C6, F-L9, F-K11 and F-K12 closed 2026-09-20/21.** Binding, cursor narrowing and activation all run through `Kernel` (`bind_by_name_on`, `narrow_cursors_on`); a `TraversalStream` carries the engine of the kernel that opened it, so a traversal opened on a native kernel activates natively instead of dropping to the interpreter; `KernelLookup` adapts any `&dyn Kernel` to `Lookup`, so a host interpolates against the kernel it holds whatever engine that is, which polydat used to make every host write for itself; and opening a traversal resolves its sources against a `Layered`/`NoScope` pair rather than a whole interpreter state over the body's program. **F-K5(b) closed 2026-09-21.** `finalize` asked its parent five questions, four of them properties of an `Arc<PolydatProgram>` and the fifth a list of cell handles, and took an `Arc<ScopeKernel<P>>` to ask them — so `build_subscope`, whose caller holds a plain `PolydatKernel`, cloned a whole kernel state per subscope to stand one up and dropped it at the end of the call. It takes a `ParentView` now, the transient is gone, and `wrap_root_kernel` — the only constructor of a root `ScopeKernel`, kept crate-private because that transient was its one caller — is public, so a host can enter the typed scope path at all. **F-K5(c) is blocked, and on something specific.** A child's import of a parent's computed output is a live link through the output's broadcast cell, not a copy at materialization time, which `subcontext::tests::a_child_follows_its_parents_computed_output` now pins. Broadcast cells are an interpreter-only creation site by the cell model's own account (cross_fiber_invalidation.md §3.1): a compiled kernel has a cell per `shared` slot and none per output. Taking the binder to `&dyn Kernel` today would compile and would silently value-copy that import on three engines out of four. It needs §4's item 12 first, and that lands in the compiled pull path, so it needs the ladder. |
| F. One body carrier | F-L2, F-L4, F-L5, F-C13 | M | Tile projection bodies and `for` bodies are two lowerings that have diverged (settings, engine choice, error path, three compiles per tile body). Tile lowering builds a `PolydatFile` plus `BodySource` as traversals do; bodies live in a program-owned table with `program_on(engine)`; the compiler hands the node an `Arc<TileProgram>` through an opaque const instead of JSON text; `tile_encode` is retired or documented as host-callable. **F-L5 done 2026-09-20 (decision 2: keep it).** The Cranelift lowering the finding wanted a decision about is already gone; the node is registered, catalogued and works, so it stays, documented as a library node a program may call and nothing generates rather than as what the compiler emits. **F-C13 half done, half declined.** `probe_element_type` compiled a whole kernel — wire resolution, the constant fold, cone extraction under `JitMode::Auto`, native codegen, a state — per generator expression, to read one output's declared port type; it assembles and asks the assembler now. The other half, making the interpreter body one more lazy entry in `program_on`, is declined: the eager compile is what makes a bad reference in a body an error of the enclosing program rather than a failure at activation, and `program_on(Interpreter)` already returns it rather than compiling again. **F-L2 and F-L4 closed 2026-09-21, and the group with them.** They were one change. `TileProgram` now holds a `Vec<Arc<BodySource>>` — the same carrier a `for` body uses — and the compiler hands the node the whole `Arc<TileProgram>` through `ConstArg::Opaque` instead of serializing the body into a JSON string for the node to re-parse. A projection body is therefore compiled once, lazily, per engine through `BodySource::program_on`, which is what retired the text handoff, the by-text intern table behind it, and the three compiles per tile body; `from_json` survives as a thin text wrapper for a host that has only text. That also settles decision 3: a projection body runs on the engine of the kernel rendering the tile. |
| G. One macro kit | F-N1, F-N2, F-N7 | L | Three closure kits with three eligibility rules over six shape vocabularies; the Rust-type-to-slot mapping in four places, two dead; native lowering a by-name match with a positional constant contract and eleven names that are not nodes. One kit from one argument-read enum; slot I/O on the `Wire` trait; a node declares its lowering the way it declares its closure. **The dead half landed 2026-09-21.** F-N2's two dead mappings are gone: `Wire::JIT` with the `ast::JitType` enum behind it (thirty impls supplied it, nothing read it, and `PortType` already answers the question through `SlotShape`), and the `ConstSource` trait, left public after the macro started emitting a `Const<T>` extraction inline. F-N7's count was low — `classify_node` matched **ninety-two** names that are not nodes, not eleven; four arms were entirely unreachable and took four `JitOp` variants and their emitters with them, and the rest were dead alternatives, mostly un-prefixed spellings of the `__`-prefixed conversion adapters. Since the match is by name, those were a hazard rather than only clutter: a host node named `i32_to_i64` would have been given polydat's native lowering. Two names the list called dead are real nodes built through the assembler rather than the DSL (`lut_sample`, `__reg_view_*`) and stay. **What is left is the reshaping**, and all of it is ladder-gated by §2: one kit from one argument-read enum (F-N1, the macro's ~730-line block over `SlotArg`/`SlotElem`/`SlotRet`/`ConfigInner` plus the u64 kit's `JitType` and the fallible kit); the same "u64 op, else copy, else slot kit, else refuse" ladder written four times (`assembly::node_step_op`, both `build_hybrid`s, `classify_node_typed`'s tail); and `jit_constants() -> Vec<u64>`, read back by index at fifty-seven sites, where reordering a node's `Const<T>` arguments silently miscompiles its native lowering. Giving `compiled_slot` the engine it is building a kit for belongs here too — it is what §5's decision 3 needs to finish. |
| H. One classifier, one tripwire | F-C5, F-E8, F-E14, F-K6 | S | Lifecycle classification is duplicated in the cone extractor and again in the hybrid batcher; nondeterminism has four encodings; H4's validation is written three times; `EnginePlan` on the interpreter is computed by node-name prefix. **F-E14 and most of F-C5 done 2026-09-20.** `engine_plan` counts native segments by asking each node whether it stands in for a subgraph (`fusion_subgraph`), not by a prefix on its display name. The cone extractor's second lifecycle walk is deleted and calls `PolydatProgram::classify_lifecycle`; the copy knew nothing of the `volatile` modifier or of volatility's contagion. `first_dynamic_wire`'s five-name list of nondeterministic sources is replaced by the declaration it was standing in for. **One real defect fell out of the third encoding**: the never-current set is computed by the inventory when the program is built, and the output modifiers are installed after it, so a node feeding a `volatile` output was excluded from the fold (which reads the modifiers) and then cached per cycle at runtime (which did not) — half of what `evaluation_model.md` §"Non-Deterministic Nodes" states. `volatile_is_never_current.rs` shows a volatile binding returning its first value on every later cycle, and passes on the fix. Two items are left and neither is what the finding says: `scalar_ok` is now three lines over `slot_color` rather than a restatement of marshal's rules, and its `Imm2` exclusion is the cone boundary's own rule; the "second eligibility check" is the codegen attempt, which extraction is designed to be able to fail. **F-E8 done 2026-09-20.** The S9 ref-validator was written twice with skip predicates that had drifted — one skipped a `None` slot only when a step owned it, the other whenever the slot was `None` — and is now the eighteenth method in `shared_core_methods!`, on the looser predicate, since nothing is published either way. The pure tier keeps its own, which has no steps to skip. The H5 half is dropped, for the reason in §2 step 2: the arena that axiom governed is gone, and a thread-local run-depth counter is not a diagnostic this project builds. F-K6 was closed with group D, so **the group is closed**. |
| I. Legacy scaffolding out | F-N4, F-N5, F-N10, F-N9 | M | About 840 lines of empty `register_nodes!` bodies; `limit` and fourteen vector nodes hand-written where the attribute would do; `instantiate` with no user; placeholder port types in the static graph against the alignment axiom. **F-N4 and F-N5(a,c) done 2026-09-20.** Every `register_nodes!` call site in the workspace is gone; the macro stays as the registration channel a host crate uses, which is what it is for. `limit` is eight lines of attribute rather than a hand impl plus a hand `FuncSig` plus a hand `compiled_u64` plus a build arm, and `mixed_radix`'s positional radix rule moved to F-N6's `validate = <path>`. `vectors.rs` lost all four of its `macro_rules!` node families — twenty-three nodes — and the two signature macros and the hand `build_node` with them: −520 lines net. Each accessor now declares its facet in its handle argument's type (`Facet<BaseFacet>`), which is where the emitted `default_resolver` comes from, so the facet is stated once instead of in the signature and the body both. That let the invariant test's `$` loophole close: it reads every line now, where before a `macro_rules!` could write any number of hand impls past it. The None-handle diagnostic that `handle_of` carried moved to `Resolved::extract`, which is the one place all of them now go through. **F-N10 was already done** by earlier work — `no_jit`, `instantiate`, `adapter`, `VariadicElement::F64` and `emit_compiled_u64` are all gone, and the macro emits `description` and `help` from the node's doc comment, which group M's generated catalog reads. **F-N9 done 2026-09-20, closing the group**, and it was a live defect rather than drift. A node returning `Value` from a `&[Value]` variadic declared its output port `PortType::U64`, which downstream read as a fact: `matches(pick(b, "yes"), "y.*")` got a `U64ToString` adapter inserted between them, and the adapter read the string's pointer as a number, so the pattern was compared against `"2163471811744"`. The output type is resolved from the wires and handed to the constructor now, the way a `Value` argument's already was. On the input side, `Port::accepts_any_type` replaces `UNTYPED_VARIADIC_NODES`, a list of thirteen node names: the property belongs to the port, which is also the right granularity — the list disabled the check on `pick`'s `Bool` selectors too. Six of the thirteen never needed it at all, since the assembler resolves a `Value` argument's port type from its wire. `Value::None.port_type()` still answers `U64`, because no port type names an absent value; the diagnostics read `Value::type_name` instead, so a reader is told "got none" rather than sent looking for a number. |
...
| K. One grammar | F-L1, F-L7, F-L8 | M | Four grammar documents, two claiming sole authority; the tested one omits `for`, `tile`, the `if` block and `shared x: T`. One normative, test-checked file, with a test that fails when an AST variant has no example. The comprehension text front end renamed from "legacy" and `custom(fn)` removed. **Closed 2026-09-20.** F-L1 was done by the language-spec pass: `grammar.md` and `language_spec.md` are deleted, `polydat_grammar.md` is the normative file and carries the G-axioms and the productions, and the three contradictions the finding named are all resolved in it — ten hard keywords, `as` reaching the whole adapter catalog, comparisons producing `u64` 0/1. F-L7 is `every_surface_form_has_an_example_in_the_spec`, which walks every `polydat` block and compares what it finds against exhaustive matches over `Statement`, `Expr`, `ForSourceKind`, `TilePiece` and `TileBodyKind`; a new AST variant fails to compile there until named, then fails the test until the document shows it. It found one gap — the derivation `for <producer> where … order …`, which the EBNF gives and no example did. F-L8: the text front end is `clause_ast` / `from_clauses` / `clauses_to_algebra` rather than "legacy", `custom(<fn>)` is a parse error from one message that says the strategy set is closed, and an order spec parses through `spec::parse_order` instead of a `__o in 0..1 order <spec>` carrier run through the whole comprehension parser. |
...
| M. Generated node reference | F-N12, F-N13 | S | The hand-curated half of `nodes.md` is wrong about the library (counts, missing files, host nodes documented as polydat's); `library_catalog.md` is an older second catalog. Generate the listing from the registry once the macro carries help text. **Done 2026-09-20.** The catalog is generated by `tests/nodes_reference.rs` between markers, the way the engine matrix above it already was: 355 callable nodes in 24 categories, each with its own first doc paragraph, and one line for the 355 `__`-prefixed adapters a program cannot name. The hand-curated category sections and the source-layout table are deleted — the table's counts were wrong for six files and omitted fourteen — and the prose introduction is what is left by hand. The macro already emits `description` from the node's doc comment, so F-N10's "empty help text" was stale in premise; 80 of the 355 render blank, which is now a visible gap in the page rather than an invisible one in the registry. F-N13 was closed by earlier weeding: `library_catalog.md` states that it is the design rationale and not a listing, and none of the signatures the finding named survive in it. |
...
2. ~~**F-K1 with the H5 tripwire** (group H's debug run-depth counter lands here). One
   day.~~ **The tripwire half is dropped, 2026-09-20.** H5 belonged to the handle arena,
   which the compiled by-reference slots design removed; there is no flag left to assert
   about. The counter it proposed was a debug-only thread-local run depth, which is the
   shape of diagnostic this project does not build — a diagnostic is a first-class feature
   on an owned object, not a process-global side table that only exists under a test
   harness.
...
10. Tiles after SRD 117: hole values as the render node's inputs; a projection body is a
    `BodySource` the compiler hands the node inside the `Arc<TileProgram>`, compiled
    lazily per engine through `program_on` and so on the engine of the kernel that
    renders the tile (*restated 2026-09-21; the first wording, precompiled at
    construction on `Engine::default()` because a closure has no engine to ask, was true
    of the text handoff group F retired*); memoized tuples and their condition; integers
    and floats written directly,
...
2. ~~**`tile_encode`**: keep as a host-callable library node with its lowering, or delete
   with `JitOp::TileEncode`. Nothing in the DSL emits it. (F-L5)~~ **Decided 2026-09-20:
   keep it.** The premise was half stale — the Cranelift lowering is already gone, so
   there is no `JitOp::TileEncode` to delete with it. What is left is a registered,
   catalogued node that encodes one value under one hole's spec and works. It is
   documented as that now: a library node a program may call and nothing generates, rather
   than as the thing the compiler emits, which it stopped being when encoding moved into
   the renderer.
3. ~~**Projection bodies' engine**: on `Engine::default()` always (today, with the H5
   reason) or on the enclosing kernel's engine as `for` bodies are. (F-L2 consequence
   3)~~ **Decided 2026-09-21: the enclosing kernel's engine.** Once the compiler hands
   the node an `Arc<TileProgram>` rather than text, a projection body is a `BodySource`
...
4. ~~**The wide-extern refusal**: close it (two-slot extern seeding) or state it as the
   ...
   carrier or as the pair into the stored value, so a two-slot immediate type has no
   compiled form — and it is refused by name through `KernelError::Refused` rather than
