...
group F, and all of group E but F-K5's parts (b) and (c) have landed, and the runtime, node library, and language
...
- **Group A, second half** — reconciling `run_fresh`, `run_guarded` and `run_order`, the
  ...
  for a quiet machine.
- **Group E** — what is left is F-K5's parts (b) and (c): a `ParentView` instead of a
  transient parent kernel for `SubcontextBuilder`, and materialization expressed over
  `dyn Kernel`. F-K11, F-K12, F-L9 and F-C6 are closed.
