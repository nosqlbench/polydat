...
fn lerp(
    t: f64,
    #[poly_default(0.0f64)] a: polydat::derive_support::Const<f64>,
    #[poly_default(1.0f64)] b: polydat::derive_support::Const<f64>,
) -> f64 {
    *a + t * (*b - *a)
}
...
fn scale_range(
    input: u64,
    #[poly_default(0.0f64)] min: polydat::derive_support::Const<f64>,
    #[poly_default(1.0f64)] max: polydat::derive_support::Const<f64>,
) -> f64 {
...
fn remap(
    input: f64,
    #[poly_default(0.0f64)] in_min: polydat::derive_support::Const<f64>,
    #[poly_default(1.0f64)] in_max: polydat::derive_support::Const<f64>,
    #[poly_default(0.0f64)] out_min: polydat::derive_support::Const<f64>,
    #[poly_default(1.0f64)] out_max: polydat::derive_support::Const<f64>,
) -> f64 {
...
/// `lerp(unit_interval(x), lo, hi)` → `scale_range(x, lo, hi)`: the
/// intermediate unit interval eliminated. Registered after the hash
/// rules, so a hash upstream takes `hash_interval` first.
fn unit_lerp_to_scale_range() -> FusionRule {
    FusionRule {
...
polydat::inventory::submit! {
    FusionRuleRegistration { priority: 30, build: unit_lerp_to_scale_range }
}
