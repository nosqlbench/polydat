...
//! [`polydat::derive_support::Ext`] combinator and projects it into
...
fn random_in(partition: Ext<Partition>, seed: u64) -> u64 {
    ...
    if card == 0 {
        ...
        partition.start_ord + crate::hash::splitmix64_u64(seed) % card
    }
}
...
fn subdivide(partition: Ext<Partition>, n: u64) -> Ext<PartitionList> {
    let parts = polydat::iteration::cursor_partition::subdivide_partition(&partition, n)
...
fn partitions(
    spec: &str,
    #[poly_default(100u64)] extent: polydat::derive_support::Const<u64>,
) -> Ext<PartitionList> {
    let parsed = polydat::iteration::cursor_partition::parse(spec)
        .unwrap_or_else(|e| panic!("partitions: bad spec `{spec}`: {e}"));
    let parts = polydat::iteration::cursor_partition::resolve(&parsed, 0, *extent)
